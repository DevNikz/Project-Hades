...
public class Combat : MonoBehaviour
{
    ...
    void StateHandlerClick() {
        ...
        if(IsMouseOverGameWindow && MenuScript.weaponWheelCheck == false && gameObject.tag == "Player" && LevelTrigger.AtEndOfLevel == false && PauseMenuScript.isPausedCheck == false) {
            switch(pressed) {
                case true:
    ...
    void StateHandler(Parameters parameters) {
        ...
        //Of course it will cause an inf. loop if I set it to a while loop. Dumbass.
        //Debug.Log(MenuScript.weaponWheelCheck);
        if(IsMouseOverGameWindow && MenuScript.weaponWheelCheck == false && gameObject.tag == "Player" && LevelTrigger.AtEndOfLevel == false) {
